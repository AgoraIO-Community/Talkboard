...
/**
 @defgroup Core_Methods Core Methods
 \brief Core methods for creating an RTC engine and setting up a basic communication channel.

 This group of methods is applicable to the following Agora products:

 - Voice communication
 - Video communication
 - Interactive broadcast

 @defgroup Core_Audio Core Audio
 \brief Core audio methods.
 ...
 @defgroup Core_Video Core Video
 \brief Core video methods.
 ...
 @defgroup Audio_Misc_Control Audio Misc Control
 \brief Miscellaneous audio control methods
 ...
 @defgroup Audio_Sound_Effect Audio Sound Effect
 \brief Audio sound effect methods.
 ...
 @defgroup Audio_Mixing Audio Mixing
 \brief Audio mixing methods.
 ...
 @defgroup Audio_Effect_Playback Audio Effect Playback
 \brief Audio effect methods
 ...
 @defgroup Audio_Recorder Audio Recorder
 \brief Audio recording methods.
 ...
 @defgroup Echo_Tests Echo Tests
 \brief Echo testing methods.
 ...
 @defgroup External_Audio_Data External Audio Data
 \brief Methods for configuring the external audio data.
 ...
 @defgroup Raw_Audio_Data Raw Audio Data
 \brief Methods for configuring the raw audio data.
 ...
 @defgroup Raw_Video_Data Raw Video Data
 \brief Methods for configuring the raw video data.
 ...
 @defgroup Watermark Watermark
 \brief Methods for configuring the watermark during a live broadcast.

 This group of methods is applicable to Agora's interactive broadcast product only.

 @defgroup Stream_Fallback Stream Fallback
 \brief Video stream fallback methods.

 This group of methods is applicable to Agora's interactive broadcast product only.

 @defgroup Dual_Video_Mode Dual Video Mode
 \brief Methods for configuring the dual video mode.
 ...
 \brief Encryption-specific methods.
 ...
 @defgroup Inject_Stream_URL Inject Stream URL
 \brief Methods for injecting a stream to a specified HTTP/HTTPS URL address during a live broadcast.

 This group of methods is applicable to Agora's interactive broadcast product only.

 @defgroup CDN_Publisher CDN Live Streaming
 \brief Methods for CDN live streaming.

 This group of methods is applicable to Agora's interactive broadcast product only.

 @defgroup Data_Stream Data Stream
 \brief Methods for data streaming.
 ...
 @defgroup Video_Misc_Control Video Misc Control
 \brief Miscellaneous video control methods.
 ...
 @defgroup Misc_Methods Misc Methods
 ...
 @defgroup Customized_Methods Customized Methods
 \brief Customized methods.
 ...
 @defgroup Screen_Capture Screen Capture
 \brief Screen capture methods.
 ...
 \brief Methods for managing audio/video devices.
 ...
 @defgroup Core_Events Core Events
  \brief Core events.

 @defgroup Media_Events Media Events
 \brief Media events.

 @defgroup Device_Events Device Events
 \brief Device events.

 @defgroup Statistics_Events Statistics Events
 \brief Statistics events.

 @defgroup Raw_Media_Data_Events Raw Media Data Events
 \brief Raw media data events.

 @defgroup AudioMixing_Events Audio Mixing Events
 \brief Audio mixing events.

 @defgroup Stream_Message_Events Stream Message Events
 \brief Stream message events.
 */
...
/** Use mode of the media::IAudioFrameObserver::onRecordAudioFrame callback function. */
...
enum STREAM_FALLBACK_OPTIONS
{
    ...
    /** 1: Under poor downlink network conditions, the SDK will receive the low stream (lower resolution and lower bitrate). You can only set this option in RtcEngineParameters::setRemoteSubscribeFallbackOption. Nothing happens when you set this in RtcEngineParameters::setLocalPublishFallbackOption. */
...
struct VideoCompositingLayout
{
   ...
   */
    ...
    */
    ...
    /** Ignore this parameter. The width of the canvas is set by IRtcEngine::configPublisher, not by VideoCompositingLayout::canvasWidth.
    */
    int canvasWidth;
    /** Ignore this parameter. The height of the canvas is set by IRtcEngine::configPublisher, not by VideoCompositingLayout::canvasHeight.
    */
    int canvasHeight;
    /** RGB hex color value.
...
/** (Recommended) The standard bitrate in IRtcEngine::setVideoEncoderConfiguration.
...
/** The compatible bitrate in IRtcEngine::setVideoEncoderConfiguration.
...
////////////////////////////////////////////////////////
    ...
    /** An error occurred during SDK runtime.
     ...
     For example, the SDK reports an #ERR_START_CALL error when failing to initialize a call. The application informs the user that the call initialization failed and invokes the IRtcEngine::leaveChannel method to leave the channel.
    ...
    /** The user has successfully joined the specified channel with an assigned channel ID and user ID.
     ...
     The channel ID assignment is based on the channel name specified in the IRtcEngine::joinChannel method. If the user ID is not specified when joinChannel is called, the server assigns one automatically.

     @param channel  Channel name.
     @param  uid User ID.
     @param  elapsed Time elapsed (ms) from the user calling IRtcEngine::joinChannel until this callback function is triggered.
     */
    ...
    /** The user has left the channel.
     ...
     When the application calls the IRtcEngine::leaveChannel method, the SDK uses this callback function to notify the application that the user has successfully left the channel.
    ...
    */
    ...
    /** A remote user or a host has joined the channel.
     ...
     @param uid User ID.
     @param elapsed Time delay (ms) from the user calling IRtcEngine::joinChannel or IRtcEngine::setClientRole (BROADCASTER) until this callback function is triggered.
     */
    ...
    /** The last mile network quality of the local user.
     ...
     This callback function is triggered once every two seconds after IRtcEngine::enableLastmileTest is called.
    ...
    Once the connection is lost, the SDK attempts to reconnect until the application calls IRtcEngine::leaveChannel.
    */
    ...
    /** The connection between the SDK and the server is lost.
     ...
     If the reconnection fails within a certain period of time (10 seconds by default), the onConnectionLost callback function is triggered. The SDK attempts to reconnect until the application calls IRtcEngine::leaveChannel.
     */
    ...
    /** The Token has expired.
     ...
     This callback function notifies the application to generate a new Token. Call IRtcEngine::renewToken to renew the Token.

     In previous SDKs, this function was provided in the IRtcEngineEventHandler::onError callback function, in the #ERR_TOKEN_EXPIRED and #ERR_INVALID_TOKEN errors. Starting from the Agora SDK v1.7.3, the old method is still valid, but it is recommended to use the onRequestToken callback function.
     */
    ...
    /** The Token will expire in 30 seconds.
     ...
     If the Token specified when calling IRtcEngine::joinChannel expires, the user will become offline. This callback function is triggered 30 seconds before the Token expires to remind the application to renew the Token. Upon receiving this callback function, generate a new Token on the server and call IRtcEngine::renewToken to pass the new Token on to the SDK.
...
////////////////////////////////////////////////////////
    ...
    /** Indicates who is talking and the speaker's volume.
     ...
     This callback function is disabled by default and can be enabled by the RtcEngineParameters::enableAudioVolumeIndication method.
    ...
    If the user has enabled the audio volume indication by calling RtcEngineParameters::enableAudioVolumeIndication, this callback function returns the uid of the active speaker whose voice is detected by the audio volume detection module of the SDK.

    @note

    - To receive this callback function, you need to call RtcEngineParameters::enableAudioVolumeIndication.
    ...
    */
    ...
    @param elapsed Time elapsed (ms) from the local user calling IRtcEngine::joinChannel until this callback function is triggered.
    If IRtcEngine::startPreview is called before joinChannel, then this is the time elapsed from startPreview until this callback is triggered.
    */
    ...
    @param width Width (pixels) of the video stream.
    @param height Height (pixels) of the video stream.
    @param elapsed Time elapsed (ms) from the remote user calling IRtcEngine::joinChannel until this callback function is triggered.
    */
...
////////////////////////////////////////////////////////
    ...
    /** The camera is turned on and ready to capture the video.
     ...
     If the camera fails to turn on, fix the error reported in the IRtcEngineEventHandler::onError callback function.
     */
...
////////////////////////////////////////////////////////
    ...
    /** The audio mixing file playback has finished.
     ...
     This callback function is triggered when the audio mixing file playback is finished after calling RtcEngineParameters::startAudioMixing.

     If you failed to execute the startAudioMixing method, an error code returns in the IRtcEngineEventHandler::onError callback function.
    ...
    /** Far-end rhythm begins.
     ...
     When a remote user calls RtcEngineParameters::startAudioMixing to play the background music, the SDK reports this callback function.
     */
...
////////////////////////////////////////////////////////
    ...
    /** The local audio effect playback has finished.
     ...
     This callback function is triggered when the audio effect file playback is finished after calling RtcEngineParameters::playEffect.
...
////////////////////////////////////////////////////////
/** \addtogroup Stream_Message_Events
  @{
 */
////////////////////////////////////////////////////////
...
/** The publisher's transcoding was updated. */
   ...
   /** The locally published media stream fell back to an audio-only stream due to poor network conditions or switched back to the video after the network conditions improved.
    ...
    If you call RtcEngineParameters::setLocalPublishFallbackOption and set option as #STREAM_FALLBACK_OPTION_AUDIO_ONLY, this callback function will be triggered when the locally published stream falls back to audio-only mode due to poor uplink conditions, or when the audio stream switches back to the video after the uplink network condition improves.
    ...
    */
    ...
    /** The remotely subscribed media stream fell back to audio-only due to poor network conditions or switched back to the video after the network conditions improved.
     ...
     Once you enabled RtcEngineParameters::setRemoteSubscribeFallbackOption, this event will be triggered to receive audio only due to poor network conditions or switches back to the video after the network condition improves.
...
public:
    ...
    */
    ...
    /** Retrieves a specified piece of information about an indexed video device.
     ...
     @param index An input parameter with a specified index. Must be less than the return value of IVideoDeviceCollection::getCount.
...
////////////////////////////////////////////////////////
    ...
    /** Enumerates the video devices.
     ...
     This method returns an IVideoDeviceCollection object including all video devices in the system. With the IVideoDeviceCollection object, the application can enumerate the video devices. The application must call the IVideoDeviceCollection::release method to release the returned object after using it.
    ...
    /** Starts the video capture device test.
     ...
     This method tests whether the video-capture device works properly. Before calling this method, ensure you have already called IRtcEngine::enableVideo, and the HWND window handle of the incoming parameter is valid.
...
////////////////////////////////////////////////////////
    ...
    /** Sets a device with the device ID.
     ...
     @param deviceId Device ID of the video-capture device. Call IVideoDeviceManager::enumerateVideoDevices to retrieve it.
...
////////////////////////////////////////////////////////
    ...
    /** Retrieves the total number of audio playback or audio recording devices.
     ...
     @note You must first call IAudioDeviceManager::enumeratePlaybackDevices or IAudioDeviceManager::enumerateRecordingDevices before calling this method, to return the number of the audio playback or audio recording devices.
...
////////////////////////////////////////////////////////
    ...
    /** Specifies an audio playback device associated with the device ID.
     ...
     @param deviceId Device ID of the audio playback device retrieved by calling the IAudioDeviceManager::enumeratePlaybackDevices method.
    ...
    /** Sets the audio recording device using the device ID.
     ...
     @param deviceId Device ID of the audio recording device that may be retrieved by calling the IAudioDeviceManager::enumerateRecordingDevices method.
    ...
    /** Starts the microphone test.
     ...
     This method tests whether the microphone works properly. Once the test starts, the SDK uses the IRtcEngineEventHandler::onAudioVolumeIndication callback function to notify the application with the volume information.

     @param indicationInterval Interval period (ms) of the IRtcEngineEventHandler::onAudioVolumeIndication callback function cycle.
     @return
...
public:
    ...
    /** Sets the channel profile.
     ...
     - Only one profile can be used at the same time within the same channel. To switch to another profile, call IRtcEngine::release to release the current Engine and create a new one using createAgoraRtcEngine() and IRtcEngine::initialize before calling this method to set the new channel profile.
     - Ensure different App IDs are used for different channel profiles.
     - This method must be called and configured before IRtcEngine::joinChannel because the channel profile cannot be configured when the channel is in use.
    ...
    /** Allows a user to join a channel.
     ...
     @warning Ensure that the App ID you use for creating the Token is exactly the same one used by IRtcEngine::initialize to initialize the RTC engine, otherwise, the CDN live streaming may fail.
     ...
     @param uid (Optional) User ID. A 32-bit unsigned integer with a value ranging from 1 to 2<sup>32</sup>-1. The uid must be unique. If a uid is not assigned (or set to 0), the SDK assigns one and returns it in the IRtcEngineEventHandler::onJoinChannelSuccess callback function. Your app must record and maintain the returned value since the SDK does not maintain its value.
    ...
    /** Allows a user to leave a channel, such as hanging up or exiting a call.
     ...
     This method is called asynchronously, and the user has not actually left the channel when the call returns. Once the user leaves the channel, the SDK triggers the IRtcEngineEventHandler::onLeaveChannel callback function.

     @note If you call IRtcEngine::release immediately after leaveChannel, the leaveChannel process will be interrupted, and the SDK will not trigger the onLeaveChannel callback function.
    ...
    /** Renews the Token.
     ...
     - The IRtcEngineEventHandler::onTokenPrivilegeWillExpire callback function is triggered, or
     - The IRtcEngineEventHandler::onError callback function reports the #ERR_TOKEN_EXPIRED error, or
     - The IRtcEngineEventHandler::onRequestToken callback function reports the #ERR_TOKEN_EXPIRED error.
...
////////////////////////////////////////////////////////
    ...
    - After calling this method, always call IRtcEngine::stopEchoTest to end the test. Otherwise, the application cannot run the next echo test.
    - In the live broadcast mode, only the hosts can call this method. If the user has switched from a communication to live broadcast channel, the user must call IRtcEngine::setClientRole to change the user role from the audience (default) to the host before using this method.
...
////////////////////////////////////////////////////////
    ...
    /** @deprecated
     ...
     This method is deprecated as of v2.3. Use IRtcEngine::setVideoEncoderConfiguration instead.
     ...
     - If you do not need to set the video encoding profile after joining the channel, Agora recommends calling this method before IRtcEngine::enableVideo to reduce the time to receive the first video frame.
    ...
    /** Sets the video encoder configuration.
     ...
     @note If the user does not need to set the video encoding profile after joining the channel, Agora recommends calling this method before IRtcEngine::enableVideo to reduce the time to receive the first video frame.
    ...
    /** Starts the local video preview before joining the channel.
     ...
     @note Once startPreview is called to start the local video preview, if you leave the channel by calling IRtcEngine::leaveChannel, the local video preview remains until you call IRtcEngine::stopPreview to disable it.
...
////////////////////////////////////////////////////////
    ...
    @note This method enables the internal engine and works either within or outside the channel. This method is still valid after a user leaves the channel.
    ...
    */
    ...
    The IRtcEngineEventHandler::onMicrophoneEnabled callback function will be triggered once the local audio function is disabled or re-enabled.

     @note
     - Call this method after IRtcEngine::joinChannel.
     - This method is different from RtcEngineParameters::muteLocalAudioStream and RtcEngineParameters::muteRecordingSignal:
         - enableLocalAudio: Disables/Re-enables the local audio capturing and handling.
         - RtcEngineParameters::muteLocalAudioStream: Stops/Continues sending the local audio streams.
         - RtcEngineParameters::muteRecordingSignal: Mutes/Unmutes the sound captured by the microphone.
	...
	/** Sets the audio parameters and application scenarios.
     ...
     - setAudioProfile must be called before IRtcEngine::joinChannel.
     ...
     @param  profile Set the sampling rate, bitrate, encode mode, and the number of channels: #AUDIO_PROFILE_TYPE
     @param  scenario Set the audio application scenario: #AUDIO_SCENARIO_TYPE
...
////////////////////////////////////////////////////////
    ...
    /** Retrieves the current call ID.
     ...
     When a user joins a channel on a client, a callId is generated to identify the call from the client. Feedback methods, such as IRtcEngine::rate and IRtcEngine::complain, must be called after the call ends, so feedback can be submitted to the SDK. These methods require the callId parameter.
    ...
    */
    ...
    /**  Enables the network test.
     ...
     Call IRtcEngine::disableLastmileTest to disable the test immediately after receiving the IRtcEngineEventHandler::onLastmileQuality callback function, and before the user joins the channel.
    ...
    /** Retrieves the warning or error description.
     ...
     @return code #WARN_CODE_TYPE or #ERROR_CODE_TYPE returned in IRtcEngineEventHandler::onWarning or IRtcEngineEventHandler::onError.
     */
...
////////////////////////////////////////////////////////
    ...
    /** Adds a stream URL to where the host publishes the stream. (CDN live only.)
     ...
     The host publishes the stream to the specified CDN live HTTP/HTTPS URL address. The corresponding callback function is: IRtcEngineEventHandler::onStreamPublished.
    ...
    /** @deprecated
     ...
     @note This method is deprecated and Agora recommends using the IRtcEngine::setLiveTranscoding method. This method sets the picture-in-picture layouts for live broadcasts. This method is only applicable when you want to push streams to the Agora server. When you push the stream to the server:
...
////////////////////////////////////////////////////////
    ...
    /** Removes the watermark image from the video stream added by IRtcEngine::addVideoWatermark.
...
////////////////////////////////////////////////////////
/** \addtogroup Inject_Stream_URL Inject Stream URL
  @{
 */
////////////////////////////////////////////////////////
    ...
    /** Adds a voice or video stream URL to a live broadcast.
     ...
     - The IRtcEngineEventHandler::onStreamInjectedStatus callback function returns the inject status.
    ...
    /** Removes the voice or video stream URL address from a live broadcast.
     ...
     @note If successful, the IRtcEngineEventHandler::onUserOffline will be triggered and a stream uid of 666 will be returned.
...
////////////////////////////////////////////////////////
    ...
    /** Disables the local video.
     ...
     Call this method after IRtcEngine::enableVideo, otherwise, this method may not work properly.
    ...
    /** Stops sending the local video stream.
     ...
     @note When set to true, this method does not disable the camera, and therefore does not affect the retrieval of the local video streams. This method responds faster than RtcEngineParameters::enableLocalVideo which controls the sending of local video stream.
    ...
    /** Stops receiving a specified remote user’s video stream.
     ...
     @note If you called RtcEngineParameters::muteAllRemoteVideoStreams and set to true to stop receiving all remote video streams, ensure that muteAllRemoteVideoStreams is called and set to false before calling this method.
...
////////////////////////////////////////////////////////
    ...
    /** Starts the audio mixing.
     ...
     When the audio mixing file playback stops after calling this method, the IRtcEngineEventHandler::onAudioMixingFinished callback function will be triggered.
...
////////////////////////////////////////////////////////
/** \addtogroup Audio_Effect_Playback
  @{
 */
////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////
    ...
    /** Mutes/Unmutes the recording signal.
     ...
     Call this method after calling RtcEngineParameters::startAudioMixing to mute the recording signal from the speakerphone without muting the mixed audio.
...
////////////////////////////////////////////////////////
/** \addtogroup External_Audio_Data
  @{
 */
////////////////////////////////////////////////////////
...
//        return setObject("rtc.video.set_remote_video_stream", "{\"uid\":%u,\"stream\":%d}", uid, streamType);
    ...
    /** Sets the default video-stream type for the video received by the local user when the remote user sends dual streams.
     ...
     - If the dual-stream mode is enabled by calling RtcEngineParameters::enableDualStreamMode, the user will receive the high-video stream by default. This method allows the application to adjust the corresponding video-stream type according to the size of the video window, reducing the bandwidth and resources.
     - If the dual-stream mode is not enabled, the user will receive the high-video stream by default.

     The result after calling this method will be returned by IRtcEngineEventHandler::onApiCallExecuted. The Agora SDK receives the high-video stream by default, to reduce the bandwidth. If needed, users can switch to the low-video stream through this method.
...
////////////////////////////////////////////////////////
/** \addtogroup Raw_Audio_Data
  @{
 */
////////////////////////////////////////////////////////

    /** Sets the audio recording format for the media::IAudioFrameObserver::onRecordAudioFrame callback function.
    ...
    /** Sets the audio playback format for the agora::media::IAudioFrameObserver::onPlaybackAudioFrame callback function.
    ...
    /** Sets the mixed audio format for the agora::media::IAudioFrameObserver::onMixedAudioFrame callback function.
...
////////////////////////////////////////////////////////
/** \addtogroup Video_Misc_Control Video Misc Control
  @{
 */
////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////
/** \addtogroup Stream_Fallback Stream Fallback
  @{
 */
////////////////////////////////////////////////////////
    ...
    /** Sets the fallback option for the remotely subscribed stream based on the network conditions.
     ...
     Once the locally published stream falls back to audio only or the audio stream switches back to the video stream, the IRtcEngineEventHandler::onRemoteSubscribeFallbackToAudioOnly callback function will be triggered.
