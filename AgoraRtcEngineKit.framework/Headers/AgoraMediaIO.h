 ...
 * All methods in the [AgoraVideoSourceProtocol]([AgoraVideoSourceProtocol]) are callback function. The media engine maintains a finite state machine and uses these functions at the right time. Do not use these functions directly in the app.
...
/** Initializes the video source.
 ...
 When the video source is initialized, the media engine will initialize the AgoraVideoFrameConsumer pointer in AgoraVideoSourceProtocol, then the video source will send the video frame to the media engine through the AgoraVideoFrameConsumer method.
...
/** Enables the video source.
 ...
 This callback function is called when the media engine is ready to start video streaming. You should start the video source to capture the video frame. Once the frame is ready, use the AgoraVideoFrameConsumer to consume the video frame.
...
/** Releases the video source.
 ...
 This callback function is called when AgoraVideoFrameConsumer is released by the media engine. You can now release the video source as well as AgoraVideoFrameConsumer.
 */
- (void)shouldDispose;
...
/** Defines a set of protocols to implement the custom video sink and pass it to the underlying media engine to replace the default video sink.
 ...
 AgoraVideoSinkProtocol consists of the following callback functions:
 ...
 * (Optional) Outputs the Video in the Raw Data ([renderRawData](renderRawData:size:rotation:))
 ...
 Note: All methods defined in AgoraVideoSinkProtocol are callback functions. The media engine uses these functions to inform the customized renderer of its internal changes.
 ...
 6. The media engine will call functions in AgoraVideoSinkProtocol according to its internal state.
 */
 ...
 * YES: If the video sink is ready. The media engine will then provide the video frame to the custom video sink by calling the AgoraVideoFrameConsumer interface.
